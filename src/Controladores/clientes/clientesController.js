...
const clienteCRUD = async (req, res) => {
...
const getClientesByID = async (req, res) => {
    ...
    try {
      const result = await pool.query('SELECT crm_clientepersona_v1($1, $2, $3, $4)',
        [obj, opcion, _limite, _offset]
      );
      const respuesta = result.rows[0].crm_clientepersona_v1;
      if (respuesta.status === 'ok' && respuesta.code === 200) {
...
const get_ClientesSearchCRUD = async (req, res) => {
  const obj1 = req.query
...
const getClientesCRUD = async (req, res) => {
  ...
  const clientes_verifica = async (req, res) => {
    console.log(req.query)
...
//eliminar registro
const delete_clientesCRUD = async (req, res) => {
  const { pers_id,opcion, _limite , _offset} = req.query;
  const obj = {pers_id:pers_id}
  console.log(req.query)
  try {
