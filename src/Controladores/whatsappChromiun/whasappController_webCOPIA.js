...
const initializeClient = () => {
    ...
    client.on('message', async (msg) => {
        ...
        const keywords = {
            hola: '👋 ¡Bienvenido estimado! Soy el bot de soporte de *Nuestra Empresa*. ¿En qué puedo ayudarte hoy?',
            producto: async (msg, userState) => {
                userState.step = 'start';
                await client.sendMessage(
                    msg.from,
                    '📦 Para brindarte información sobre productos, primero necesitamos que estés registrado. 😊 Comencemos con tus datos. Por favor, dime tu *nombre*.'
                );
        ...
        if (msg.type === 'chat') {
            ...
            if (respuesta.status === 'ok' && respuesta.code === 200) {
                ...
                switch (userState.step) {
                    case 'start': {
                        const msg0p = `Bienvenido a ${dataEmp.emp_nombre} \nDirección: ${dataEmp.emp_camp1}\nEn un momento te atenderemos.`;
                        const msg1p = 'Ayúdanos con algunos datos para brindarte una mejor experiencia. ¿Cuál es tu nombre?';
                
                        await client.sendMessage(msg.from, msg0p);
                        await client.sendMessage(msg.from, msg1p);
                        userState.step = 'asking_name';
                        break;
                    }
                    ...
                    case 'asking_name': {
                        ...
                        const msg2p = `Gracias ${userState.data.name}, ¿Cuál es tu apellido?`;
                
                        await client.sendMessage(msg.from, msg2p);
                    ...
                    case 'asking_lastname': {
                        ...
                        const msg3p = `Gracias, ${userState.data.name} ${userState.data.lastName}, ¿Desde dónde nos escribes?`;
                
                        await client.sendMessage(msg.from, msg3p);
                    ...
                    case 'asking_ciudad': {
                        ...
                        const msg4p = `Gracias, ${userState.data.name} ${userState.data.lastName}, ¿puedes proporcionarnos tu correo electrónico?`;
                
                        await client.sendMessage(msg.from, msg4p);
